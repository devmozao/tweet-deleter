...
it('Should render the tweet', () => {
  const component = render(<Tweets tweetdata={payload} />)
  expect(component.getByText('@tchublim to te aguardando! chega aí!'))
})

it('Should select the tweet and change the dom with the selected effect', () => {
  const component = render(<Tweets tweetdata={payload} />)

  fireEvent.click(component.getByText('@tchublim to te aguardando! chega aí!'))

  expect(component.getByTestId(payload.data[0].id_str.toString()))
})
